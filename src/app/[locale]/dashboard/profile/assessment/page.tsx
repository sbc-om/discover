...
const RadialInsight = ({ label, value }: { label: string; value?: number | null }) => {
  ...
  return (
    <div className="rounded-xl bg-gradient-to-r from-zinc-700 to-zinc-800 border border-zinc-700 p-3 text-center">
      <svg width="46" height="46" className="mx-auto">
        <circle cx="23" cy="23" r={radius} stroke="currentColor" strokeWidth="4" className="text-zinc-600" fill="transparent" />
        ...
        <text x="23" y="27" textAnchor="middle" className="fill-white text-[10px] font-semibold">{value ?? 0}</text>
      </svg>
    ...
    <div className="mx-auto w-full max-w-[390px] space-y-4">
      ...
      {loading ? (
        ...
        <>
          {/* Request Status */}
          {requestStatus && (
            <div className="rounded-2xl border border-zinc-800 bg-gradient-to-r from-zinc-800 to-zinc-900 p-4 space-y-3">
          ...
          {latestTest && latestTest.status === 'completed' && (
            <div className="rounded-2xl border border-zinc-800 bg-gradient-to-r from-zinc-800 to-zinc-900 p-4 space-y-4">
                ...
                <div className="flex items-center gap-2">
                  {canSeeAllTests && (
                    <button
                      ...
                      className="h-7 w-7 rounded-full border border-zinc-700 flex items-center justify-center text-zinc-400 hover:text-white"
                    >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl bg-gradient-to-r from-zinc-700 to-zinc-800 border border-zinc-700 p-3">
                  <p className="text-[10px] uppercase tracking-widest text-zinc-400">{isAr ? 'طول' : 'Height'}</p>
                  <p className="text-sm font-semibold text-white">{latestTest.height || '-'} cm</p>
                </div>
                <div className="rounded-xl bg-gradient-to-r from-zinc-700 to-zinc-800 border border-zinc-700 p-3">
                  <p className="text-[10px] uppercase tracking-widest text-zinc-400">{isAr ? 'وزن' : 'Weight'}</p>
                  <p className="text-sm font-semibold text-white">{latestTest.weight || '-'} kg</p>
                </div>
                <div className="rounded-xl bg-gradient-to-r from-zinc-700 to-zinc-800 border border-zinc-700 p-3">
                  <p className="text-[10px] uppercase tracking-widest text-zinc-400">{isAr ? 'ضغط' : 'Blood'}</p>
                  <p className="text-sm font-semibold text-white">{latestTest.blood_pressure || '-'}</p>
                </div>
                <div className="rounded-xl bg-gradient-to-r from-zinc-700 to-zinc-800 border border-zinc-700 p-3">
                  <p className="text-[10px] uppercase tracking-widest text-zinc-400">{isAr ? 'نبض' : 'Heart'}</p>
                  <p className="text-sm font-semibold text-white">{latestTest.heart_rate || '-'} bpm</p>
                </div>
              </div>
              ...
              {canSeeTestReviewNotes && latestTest.notes && (
                <div className="rounded-xl bg-gradient-to-r from-zinc-700 to-zinc-800 border border-zinc-700 p-3">
                  <p className="text-[10px] uppercase tracking-widest text-zinc-500">{isAr ? 'ملاحظة الاختبار' : 'Test note'}</p>
                  <p className="mt-2 text-xs text-zinc-300">{latestTest.notes}</p>
                </div>
              )}
            </div>
          )}
          ...
          {canSeeAllTests && showAllTests && (
            <div className="space-y-3">
              ...
              {allTestsLoading ? (
                ...
                allTests.map((test) => (
                  <div key={test.id} className="rounded-2xl border border-zinc-800 bg-gradient-to-r from-zinc-800 to-zinc-900 p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-semibold text-white">{formatDate(test.test_date || test.scheduled_at, locale)}</p>
          ...
          {!latestTest && !requestStatus && (
            <div className="rounded-2xl border border-zinc-800 bg-gradient-to-r from-zinc-800 to-zinc-900 p-8 text-center">
